...
We regard agent-based test generation as a new form of model-based test generation.\todo{Hier weiter.}
...
% \todo[inline]{more/stronger motivation - why agents? what is the challenge/problem?}
...
The environment is initialised with the agents spawned randomly on any valid pavement area (see note below). When the environment is reset new random locations are set for the agents. Control over random locations are set using a fixed seed based on the experiment number and so tests are repeatable between agent types, i.e. An experiment with 5 agents using a random method with have the same starting locations as using any other method which ensures valid comparison between agent types.

The precondition is activated if an agent is found in the `braking zone' of the AV which extends 6 units forwards of the leading edge of the vehicle,~Fig.\ref{gridRoad}. %Any agent found in this area is defined as a valid test and the environment is reset.
...
For each test generated that met the precondition, the CPU time taken to execute the agent action was averaged over the $k$-tests and compared across different agent types and numbers, Fig.~\ref{CPUTime}. This is essentially comparing the resources required to execute the actions of differenet agent types.
...
% \textbf{Method} & Agent Description & $CL$ & Accuracy & $CS$ &  $t_{CPU}$ (s) & $TGT$ \\
\hline
...
\begin{itemize}
	...
	%
	\item \textit{Exemplary}: An exemplary test case will test multiple aspects of the DUT thereby reducing the number of tests. In this case study, only a single assertion was used so this criteria cannot be assessed although it is a key development for future work discussed in Section~\ref{s:future}.
	%
...
%----------------- Conclusion ---------------------------
\section{Conclusion and Future Work}\label{s:future}
