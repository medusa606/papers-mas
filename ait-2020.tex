...
%
%\todo{Check this!}
Thus, in this case study, unavoidable collisions are those with a time to collision (TTC) of less than 1.33s. % based on 12m
%
...
The second class of behaviours is agency-directed, taking into account the agent's perceptions for strategic planning. Agents are initially walking along the pavement as in constrained random mode. The \textit{proximity} behaviour instructs the agent to cross the road when the AV is within a certain radius. Using the \textit{intersect} behaviour, the agent calculates an intersection time with the AV's precondition zone; it chooses to cross the road if it can reach an intersection point before the AV has passed. The \textit{election} behaviour ranks the probability of each agent within a multi-agent setting to enact the intersect behaviour; it elects the agent with the highest likelihood of success.
